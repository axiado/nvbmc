...
python do_generate_static() {
    ...
    bb.build.exec_func("do_mk_static_actual_size", d)

    bb.build.exec_func("do_mk_static_symlinks", d)
}

do_mk_static_actual_size() {
    if [ "${IMAGE_SIZE_ACTUAL}" != "0" ]; then
        dd if=${IMGDEPLOYDIR}/${IMAGE_NAME}.static.mtd \
        of=${IMGDEPLOYDIR}/${IMAGE_NAME}.static.actual.mtd \
        bs=1024 count=${IMAGE_SIZE_ACTUAL}
    fi
}
